...
function SetPlayerSpeed(pn, mode, speed)
	local player_data = notefield_prefs_config:get_data(pn)  
	player_data.speed_mod = speed 
	if mode == 1 or mode == "X" then
		player_data.speed_type = "multiple" 
	elseif mode == 2 or mode == "C" then
		player_data.speed_type = "constant" 
	elseif mode == 3 or mode == "M" then
		player_data.speed_type = "maximum" 
	end
...
function GetSpeedAndMode(pn)
	...
	local speed = player_data.speed_mod;
	local mode = nil

	if player_data.speed_type == "maximum" then
		mode = "M"
	elseif player_data.speed_type == "constant" then
		mode = "C"
	else
		mode = "X"
	end
...
for pn in ivalues(GAMESTATE:GetHumanPlayers()) do
	...
	t[#t+1] = Def.ActorFrame{
		...
		StateChangedMessageCommand=function(self)
			...
			if Global.state == "SpeedMods" and SideJoined(pn) then
